...
//FUNCTION - REAL SHIT
const TokenModal = ({ open, onClose, setSelectedToken, onSelectChange, isInput, tokenSetter}) => {
  ...
  useEffect(() => {
    ...
    setFilteredTokens(
      tokens.filter((token) =>
        token.name.toLowerCase().includes(searchTerm.toLowerCase()) || token.address.toLowerCase() === searchTerm.toLowerCase()
      )
    );
  }, [searchTerm, tokens]);
        ...
        <StyledBox>
          ...
          <TokenList>
            {filteredTokens.map((token) => (
              <TokenListItem key={token.address} onClick={() => handleTokenClick({
                name: token.name,
                ...
                <TokenListItemText
                  primary={
                    <>
                      {token.logoURI && (
                        <img className="logo" src={token.logoURI} alt={`${token.name} Logo`} />
                      )}
                      {token.name}
                    </>
                  }
                  secondary={<span className="address">{token.address}</span>}
                />
              </TokenListItem>
            ))}
          </TokenList>
