...
const getContractLogEvents = async (signature: string) => {
  ...
  const provider = new ethers.providers.JsonRpcProvider('https://eth-mainnet.g.alchemy.com/v2/oQcwBviequQgqzvgAXUShhOlCzgZKtKK');
  ...
  try {
    ...
    const filteredEvents = events.filter(event => event.args && event.args._signature === signature);
